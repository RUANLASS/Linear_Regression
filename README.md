...
3. data_2.py + linear_reg_2.py =>
   ...
   - used values of error on training set and cross validation set to figure out whether the model had high bias or variance.
   - model gave mean error of around 29-30% on test set.
  
4. data_3.py + linear_reg_3.py =>
   - since error/cost on training set and cross validation set seemed to be close to each other (approx. 0.011 and 0.014 respectively), implemented diagnostics to fix high bias.
   - added polynomial features to the model.
   - performed hyperparameter tuning on alpha (learning rate) and lambda (regularization strength) to try and improve the model
